...
const walkNode = (node, interpolation, guard = { slots: new Set(), forDepth: 0 }) => {
  if (node.nodeName === 'lw-slot') {
    if (guard.forDepth > 0) {
      throw new Error('lw-slot cannot sit inside an lw-for element: the template is extracted and cloned per row, so projected content would be lost. Move the slot out of the loop.');
    }
    const slotName = node.attrs?.find(a => a.name === 'name')?.value || '';
    if (guard.slots.has(slotName)) {
      throw new Error(slotName
        ? `A component template can have at most one lw-slot per name: duplicate name="${slotName}".`
        : 'A component template can have at most one default (unnamed) lw-slot. Give the others a name attribute.');
    }
    guard.slots.add(slotName);
  }
  const repeated = !!node.attrs?.some(a => a.name === 'lw-for');
  if (repeated) {
    guard.forDepth++;
  }
  node.attrs && node.attrs.forEach(attr => {
    ...
    const key = `${++astKey}`;
    if (attr.name === 'lw-context' || attr.name === 'lw-for-parent') {
      ...
      if (!matched) {
        throw new Error(`Invalid lw-for expression: "${attr.value}". Expected "item in items" or "item, index in items".`);
      }
      ...
      if (attr.name === 'lw-key' && !attr.value.trim()) {
        throw new Error(`lw-key requires an expression, e.g. lw-key="item.id" (or lw-key="item" to key by the item itself).`);
      }
      node.attrs.push({ name: 'lw-elem', value: '' });
      const ast = getAST(attr.value);
      removeASTLocation(ast);
  ...
  node.childNodes && node.childNodes.forEach(childNode => walkNode(childNode, interpolation, guard));
  if (repeated) {
    guard.forDepth--;
  }
};
