...
const walkNode = (node, interpolation) => {
   node.attrs && node.attrs.forEach(attr => {
      ...
      if (attr.name === 'lw') {
         ...
         const astItems = parser.parse(itemsExpr).program.body;
         removeASTLocation(astItems);
         interpolation[key] = { astItems, loc, itemExpr, indexExpr, itemsExpr };
         node.attrs.push({ name: 'lw-false', value: '' });
         attr.value = key;
      } else if (attr.name === ('lw-model')) {
         const ast = parser.parse(attr.value).program.body;
         removeASTLocation(ast);
         interpolation[key] = { ast, loc };
         const astModel = ast[0].expression;
         if (astModel.type === 'MemberExpression') {
            interpolation[key]['astObject'] = astModel.object;
            interpolation[key]['propertyExpr'] = astModel.property.name;
         } else if (astModel.type === 'Identifier') {
            interpolation[key]['propertyExpr'] = astModel.name;
         }
         attr.value = key;
